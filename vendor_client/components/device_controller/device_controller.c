...
static ac_device_info_t s_ac_devices[DEVICE_CONTROLLER_MAX_DEVICES];
static ac_device_info_t s_sync_device_info;
static dc_device_info_t s_placeholder_device;
static dc_device_info_t s_work_device;
static dc_device_info_t s_all_device;
static dc_device_info_t s_callback_device;
    ...
    if (feature_id) {
        ...
        if (ret == ESP_OK) {
            ...
            ESP_LOGI(TAG, "Successfully sent feature 0x%04X to device 0x%04X: %s",
                     feature_id, device_addr, msg);
        } else {
            dc_ui_integration_show_message("SET ERROR", 1000);
            ESP_LOGE(TAG, "Failed to send feature 0x%04X to device 0x%04X: %s (error: %s)",
                     feature_id, device_addr, msg, esp_err_to_name(ret));
        }
        return ret;
    }
...
static void ac_device_status_callback(uint16_t device_addr, ac_status_type_t status_type, uint8_t value)
{
    ...
    for (uint8_t i = 0; i < device_count; i++) {
        if (get_device_addr_by_index(i) == device_addr) {
            ...
            if (current_device) {
                s_callback_device = *current_device;

                switch (status_type) {
                    case AC_STATUS_POWER:
                        s_callback_device.status.power = value;
                        break;
                    case AC_STATUS_TEMPERATURE:
                        s_callback_device.status.temperature = value;
                        break;
                    case AC_STATUS_MODE:
                        s_callback_device.status.mode = value;
                        break;
                    case AC_STATUS_FAN_SPEED:
                        s_callback_device.status.fan_speed = value;
                        break;
                }

                uint16_t feature_id = ac_status_to_feature_id(status_type);
                sh_feature_state_t *state = sh_model_find_state(s_callback_device.feature_states,
                                                                s_callback_device.feature_state_count,
                                                                feature_id);
                if (!state && s_callback_device.feature_state_count < SH_MODEL_MAX_FEATURES) {
                    state = &s_callback_device.feature_states[s_callback_device.feature_state_count++];
                    state->feature_id = feature_id;
                    state->type = (status_type == AC_STATUS_TEMPERATURE) ?
                        SH_FEATURE_TYPE_INT : SH_FEATURE_TYPE_ENUM;
                }
                if (state) {
                    state->value = value;
                }

                dc_state_machine_set_device_info(i, &s_callback_device);
...
static void ac_device_online_callback(uint16_t device_addr, bool is_online)
{
    ...
    for (uint8_t i = 0; i < device_count; i++) {
        if (get_device_addr_by_index(i) == device_addr) {
            ...
            if (current_device) {
                s_callback_device = *current_device;
                s_callback_device.is_online = is_online;
                dc_state_machine_set_device_info(i, &s_callback_device);
...
static uint16_t get_feature_id_for_device_param(uint8_t device_id, dc_parameter_t param)
{
    ...
    const dc_device_info_t *device = dc_state_machine_get_device_info(device_id);
    if (device && device->profile && param < device->profile->feature_count) {
        return device->profile->features[param].feature_id;
    }
...
static esp_err_t initialize_multiple_devices(void)
{
    ...
    if (device_count == 0) {
        ...
        memset(&s_placeholder_device, 0, sizeof(s_placeholder_device));
        s_placeholder_device.device_id = 0;
        s_placeholder_device.device_name = "No Device";
        s_placeholder_device.is_online = false;
        s_placeholder_device.profile = sh_profile_ac_get();
        s_placeholder_device.status.power = false;
        s_placeholder_device.status.temperature = 22;
        s_placeholder_device.status.mode = DEVICE_CONTROLLER_MODE_AUTO;
        s_placeholder_device.status.fan_speed = DEVICE_CONTROLLER_FAN_SPEED_LOW;

        if (s_placeholder_device.profile) {
            size_t state_count = 0;
            sh_model_default_states(s_placeholder_device.profile, s_placeholder_device.feature_states,
                                    SH_MODEL_MAX_FEATURES, &state_count);
            s_placeholder_device.feature_state_count = (uint8_t)state_count;
        }

        ret = dc_state_machine_set_device_info(0, &s_placeholder_device);
        if (ret != ESP_OK) {
    ...
    memset(s_ac_devices, 0, sizeof(s_ac_devices));
    uint8_t actual_count = ac_get_device_list(s_ac_devices, DEVICE_CONTROLLER_MAX_DEVICES);
    ...
    for (uint8_t i = 0; i < actual_count; i++) {
        ...
        ESP_LOGI(TAG, "  Address: 0x%04X", s_ac_devices[i].addr);
        ESP_LOGI(TAG, "  Online: %s", s_ac_devices[i].is_online ? "YES" : "NO");
        ESP_LOGI(TAG, "  Raw name length: %d", strlen(s_ac_devices[i].device_name));

        // Print device name character by character to debug encoding issues
        ESP_LOGI(TAG, "  Raw name bytes:");
        for (int j = 0; j < 16 && s_ac_devices[i].device_name[j] != '\0'; j++) {
            ESP_LOGI(TAG, "    [%d]: 0x%02X ('%c')", j,
                    (unsigned char)s_ac_devices[i].device_name[j],
                    isprint((unsigned char)s_ac_devices[i].device_name[j]) ? s_ac_devices[i].device_name[j] : '?');
        }
        ESP_LOGI(TAG, "  Name string: \"%s\"", s_ac_devices[i].device_name);
    }
    ...
    for (uint8_t i = 0; i < actual_count; i++) {
        ...
        if (strlen(s_ac_devices[i].device_name) > 0) {
            // Check if name contains printable characters
            bool is_printable = true;
            for (int j = 0; j < strlen(s_ac_devices[i].device_name); j++) {
                if (!isprint((unsigned char)s_ac_devices[i].device_name[j])) {
                    is_printable = false;
                    break;
                }
            }
            ...
            if (is_printable) {
                ...
                strncpy(cleaned_names[i], s_ac_devices[i].device_name, sizeof(cleaned_names[i]) - 1);
        ...
        memset(&s_work_device, 0, sizeof(s_work_device));
        s_work_device.device_id = i;
        s_work_device.device_name = device_name;
        s_work_device.is_online = s_ac_devices[i].is_online;
        s_work_device.profile = s_ac_devices[i].profile;
        s_work_device.feature_state_count = s_ac_devices[i].feature_state_count;
        s_work_device.status.power = s_ac_devices[i].power_state;
        s_work_device.status.temperature = s_ac_devices[i].temperature;
        s_work_device.status.mode = s_ac_devices[i].mode;
        s_work_device.status.fan_speed = s_ac_devices[i].fan_speed;
        memcpy(s_work_device.feature_states, s_ac_devices[i].feature_states, sizeof(s_work_device.feature_states));

        ret = dc_state_machine_set_device_info(i, &s_work_device);
        if (ret != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Initialized device %d: \"%s\" (0x%04X) - %s",
                i, device_name, s_ac_devices[i].addr,
                s_work_device.is_online ? "ONLINE" : "OFFLINE");
    }
    ...
    if (actual_count > 0 && actual_count < DEVICE_CONTROLLER_MAX_DEVICES) {
        memset(&s_all_device, 0, sizeof(s_all_device));
        s_all_device.device_id = actual_count;  // 使用实际设备数量作为虚拟设备的索引
        s_all_device.device_name = AC_ALL_DEVICE_NAME;
        s_all_device.is_online = true;  // 虚拟设备始终在线
        s_all_device.profile = actual_count > 0 ? s_ac_devices[0].profile : sh_profile_ac_get();
        s_all_device.status.power = false;     // 默认状态
        s_all_device.status.temperature = 25;  // 默认温度
        s_all_device.status.mode = DEVICE_CONTROLLER_MODE_AUTO;
        s_all_device.status.fan_speed = DEVICE_CONTROLLER_FAN_SPEED_LOW;

        if (s_all_device.profile) {
            size_t state_count = 0;
            sh_model_default_states(s_all_device.profile, s_all_device.feature_states,
                                    SH_MODEL_MAX_FEATURES, &state_count);
            s_all_device.feature_state_count = (uint8_t)state_count;
        }

        ret = dc_state_machine_set_device_info(actual_count, &s_all_device);
        if (ret != ESP_OK) {
